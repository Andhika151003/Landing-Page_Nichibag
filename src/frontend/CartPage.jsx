...
const CartPage = () => {
  ...
  const handleCheckout = () => {
    const phoneNumber = "6287788261298"; // Sesuaikan nomor WA
    ...
    cart.forEach((item, index) => {
      ...
      message += `   - Harga: Rp${(item.discountPrice || item.price).toLocaleString("id-ID")}\n\n`;
    });
  ...
  if (cart.length === 0) {
    return (
      <div className="min-h-screen bg-[#F9F6EE] pt-32 px-4 flex flex-col items-center justify-center text-center">
        ...
        <h2 className="text-2xl font-bold text-gray-800">Keranjang Belanjamu Kosong</h2>
        <p className="text-gray-500 mb-8">Yuk isi dengan kemasan cantik dari Nichibag!</p>
        <Link to="/katalog" className="bg-red-600 text-white px-6 py-3 rounded-full font-bold hover:bg-red-700 transition">
          Mulai Belanja
        </Link>
      </div>
    );
  ...
  return (
    <div className="min-h-screen bg-[#F9F6EE] pt-28 pb-10 px-4 md:px-8">
      <div className="max-w-6xl mx-auto">
        <div className="mb-6">
          <Link to="/katalog" className="text-gray-500 hover:text-red-600 flex items-center gap-2 font-medium w-fit">
            ...
            {cart.map((item) => (
              ...
              >
                ...
                <div className="flex-1">
                  ...
                  <p className="text-sm text-gray-500">Warna: <span className="font-medium text-gray-700">{item.selectedColor?.name}</span></p>
                ...
                <div className="flex flex-col items-end gap-3">
                  {/* 3. Tombol Hapus dengan fungsi baru */}
                  <button 
                    onClick={() => handleDeleteItem(item._id, item.selectedColor?.name)}
                  ...
                  {/* 4. Input Jumlah dengan Warna Pink & Merah (Sesuai Request) */}
                  <div className="flex items-center bg-[#f8d7d0] shadow-sm rounded-full px-2 py-1 gap-1">
                    <button 
                      onClick={() => updateQuantity(item._id, item.selectedColor?.name, -1)} 
                      className="p-1 text-red-800 hover:bg-white/50 rounded-full transition-all active:scale-90"
                    >
                      <Minus size={16} strokeWidth={2.5} />
                    </button>
                    <span className="font-extrabold text-sm w-6 text-center text-red-900 select-none">
                      {item.quantity}
                    </span>
                    <button 
                      onClick={() => updateQuantity(item._id, item.selectedColor?.name, 1)} 
            ...
            <div className="bg-white p-6 rounded-xl shadow-md sticky top-28 border border-gray-100">
              ...
              <div className="space-y-3 mb-6 text-gray-600">
                <div className="flex justify-between text-sm">
                  ...
                  <span className="font-medium">{cart.reduce((a, c) => a + c.quantity, 0)} pcs</span>
                </div>
