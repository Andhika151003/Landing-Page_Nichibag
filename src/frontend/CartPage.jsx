...
const CartPage = () => {
  const { cart, removeFromCart, updateQuantity, getTotalPrice } = useCart(); // ... (Fungsi handleDeleteItem tetap sama)
  ...
  const handleCheckout = () => {
    ...
    cart.forEach((item, index) => {
      message += `${index + 1}. *${item.name}* \n`; // >>> PERBAIKAN UTAMA: Tambahkan Kode Produk
  ...
  return (
    <div className="min-h-screen bg-[#F9F6EE] pt-28 pb-10 px-4 md:px-8">
      ...
      <div className="max-w-6xl mx-auto">
                {/* ... (Header dan Link Kembali) */}       {" "}
        <div className="mb-6">
                   {" "}
          <Link
            to="/katalog"
            className="text-gray-500 hover:text-red-600 flex items-center gap-2 font-medium w-fit"
          >
        ...
        <div className="flex flex-col lg:flex-row gap-8">
                    {/* List Produk (Tidak perlu perubahan pada bagian ini) */} 
                 {" "}
              ...
              >
                ...
                <div className="flex-1">
                  ...
                  <p className="text-red-600 font-bold mt-1">
                                        Rp
                    {(item.discountPrice || item.price).toLocaleString("id-ID")}
                                     {" "}
                  </p>
                                 {" "}
                </div>
